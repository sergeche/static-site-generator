...
module.exports = function(fileList, options) {
	...
	// Step 1: transform a flat file lint into a tree where each url slug
	// is a tree node
	const node = navItem => ({
		items: new Map(),
		navItem
	...
	.forEach(navItem => {
		let ctx = root.items;
		const slugs = item.url.split('/').filter(Boolean);
	...
	.filter(function(file) {
		return !file.meta || !file.meta.navHidden;
	})
	.map(function(file) {
