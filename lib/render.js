...
const debug = require('debug')('ssg:render');
...
module.exports = function(options) {
	...
	return through.obj(function(file, enc, next) {
		...
		meta(file, options)
		.then(file => {
			if (utils.getExtensions(file.relative).length < 2 && !file.meta.layout) {
				// no need in rendering, file does not have enough extensions
				// for pipeline
				return file;
			}
...
var register = module.exports.register = function(ext, fn, dest) {
	if (typeof ext === 'string') {
		ext = ext.split(/[,|]/g).map(ext => ext.trim());
	}
...
function renderChain(chain, renderers, data) {
	return new Promise(function(resolve, reject) {
		...
		var next = function(ctx) {
			...
			.then(content => next(ctx.clone({content})))
			.catch(reject);
		};
		next(ctx);
	});
